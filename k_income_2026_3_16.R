...
min(gini_income_w.vec)
max(gini_income_w.vec)


ggplot(ineq.df, aes(x=year.vec, y=gini_income_w.vec)) + geom_line(linewidth = 1.0) +geom_point(size=1.9) + scale_x_continuous(breaks = 2021:2025) +labs(x="Year", y="Gini HH income inequality index") +theme(axis.text.x = element_text(size = 14, color = "black"),  axis.text.y = element_text(size = 16, color = "black"), text = element_text(size = 20))  + scale_y_continuous(breaks = seq(0.4,0.55, 0.01)) 
