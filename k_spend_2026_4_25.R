...
#2025 begins####
setwd("C:/Oz_local_workspace_3_K-economy/k-R-Derive")#2025 only for new work local
#setwd("~/SDCPC/2025_SDCPC")# work machine
dir()
# Read transaction dataset
trans2025_1.df = readRDS("dcpc_2025_tranlevel.rds")
dim(trans2025_1.df)
names(trans2025_1.df)
(sampled_num_trans_2025 = nrow(trans2025_1.df))

# Read individual dataset
indiv2025_1.df = readRDS("dcpc_2025_indlevel.rds")
dim(indiv2025_1.df)
...
#
m1.df = left_join(trans2025_2.df, indiv2025_3.df, by = "uasid")
dim(m1.df)# num of payments (transactions)
table(m1.df$date)

# select only the needed variables
m2.df = subset(m1.df, select = c(uasid, amnt, income_hh, income_quantile, merch))
...
names(m6.df)
m6.df %>% group_by(income_quantile) %>% summarise(n_distinct(uasid))
...
(m7.df = m6.df %>% group_by(income_quantile) %>% summarise(spend = sum(amnt), num_resp = n_distinct(uasid)))
dim(m7.df)
...
#
(total_resp_2025 = nrow(m6.df[unique(m6.df$uasid), ]))# num of resp
#
...
m6g.df %>% group_by(income_quantile) %>% summarise(n_distinct(uasid))
# how many trans in each quintile?
m6g.df %>% group_by(income_quantile) %>% summarise(n())

# adding 2 variables: Aggregate spending (new variable: spend) of each quantile and number of respondents in each quantile
# Below, m7 uses selected variables for the chart. Go back to m6.df for merchant specific analysis
(m7g.df = m6g.df %>% group_by(income_quantile) %>% summarise(spend = sum(amnt), num_resp = n_distinct(uasid)))
dim(m7g.df)
...
dim(m6n.df)
# how many resp in each quintile?
m6n.df %>% group_by(income_quantile) %>% summarise(n_distinct(uasid))
# how many trans in each quintile?
m6n.df %>% group_by(income_quantile) %>% summarise(n())

# adding 2 variables: Aggregate spending (new variable: spend) of each quantile and number of respondents in each quantile
# Below, m7 uses selected variables for the chart. Go back to m6.df for merchant specific analysis
(m7n.df = m6n.df %>% group_by(income_quantile) %>% summarise(spend = sum(amnt), num_resp = n_distinct(uasid)))
dim(m7n.df)
